...
class BaseLayer(keras.layers.Layer):
    ...
    def compute_output_shape(
        self, input_shape: dict[str, tuple[int, ...]]
    ) -> dict[str, tuple[int, ...]]:
        output_shape = input_shape
        output_shape["node_state"] = (*input_shape["node_state"][:-1], self.units)
        if input_shape["edge_state"] is not None:
            output_shape["edge_state"] = (*input_shape["edge_state"][:-1], self.units)
        return output_shape

    def add_kernel(
        self, name: str, shape: tuple[int, ...], dtype: str = "float32", **kwargs
    ) -> types.Variable:
    ...
    def add_bias(
        self, name: str, shape: tuple[int, ...] = None, dtype: str = "float32", **kwargs
    ) -> types.Variable:
        return self.add_weight(
            name=name,
    ...
    def _common_weight_kwargs(self, weight_type: str) -> dict[str, types.Any]:
        initializer = getattr(self, f"{weight_type}_initializer", None)
        regularizer = getattr(self, f"{weight_type}_regularizer", None)
        regularizer = (
            None if regularizer is None else regularizer.from_config(regularizer.get_config())
        )
        constraint = getattr(self, f"{weight_type}_constraint", None)
        constraint = (
            None if constraint is None else constraint.from_config(constraint.get_config())
        )
        return {
